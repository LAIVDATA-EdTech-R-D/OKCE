...
init <- function() {
  info <- fromJSON(file = "./json/info_back.json")
...
#  rf_imp_dat = rf_imp
#  col_names = colnames(analysis_data1)
  ...
  temp2 = temp1[temp1$max_rn < 10,]
  temp2$add_rn = 10-temp2$max_rn
  
  temp3=merge(x=rf_imp_dat, y=fset, all.x=TRUE)
  temp4 = temp3[is.na(temp3$ord),]
  
  #2. make possible pair for lacking "before"s .
  temp5 = merge(temp4, temp2, by="before", all.y=TRUE)

  #3. select missing number of observations
  
  temp6 = sqldf("select *, random() as rand from temp5")
  temp7 = sqldf("select * from (select before, after,imp, add_rn, ROW_NUMBER() OVER(PARTITION BY before  ORDER BY rand) as rank from temp6) where rank <= add_rn ")
  
  # bind final set with random samples 

  fset_add = sqldf("select before, after, 4,'RF',imp,NULL from temp7")
  colnames(fset_add)= colnames(fset)
  fset1=rbind(fset,fset_add)
  fset2= fset1[order(fset1$before),] 
  return(fset2)
...
#   analysis_data <- read.csv("/home/laivdata/바탕화면/choihh/FIR/DATA/algebra_picture3000.csv")[,-1] # input file should be a matrix
    analysis_data0 <- analysis_data[rowSums(analysis_data[])<ncol(analysis_data[]) & rowSums(analysis_data[]) > 0 ,]    
    analysis_data1 <- analysis_data0[,colSums(analysis_data0[]) !=nrow(analysis_data0[])]
    print(paste0("original: ", ncol(analysis_data0), "  cleansed: ", ncol(analysis_data1)))
...
#----------------------------------Merge RF & LASSO ---------------------------------------  
    ...
    toc()
    write.csv(relation_f1,"./OUT/picture_relation_rename.csv")
